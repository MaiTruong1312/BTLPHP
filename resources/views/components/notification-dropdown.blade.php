...
<div x-data="{ 
        ...
        markAsRead(notificationId, targetUrl, event) {
            event.preventDefault();
            const currentNotificationEl = event.currentTarget;
            ...
            .then(response => {
                if (response.ok) {
                    this.unreadCount--;
                    currentNotificationEl.closest('.notification-item').remove();
                }
            ...
            <div id="notification-list">
                ...
                <div class="notification-item">
                    <a href="{{ $notification->data['url'] ?? '#' }}"
                        x-on:click="markAsRead('{{ $notification->id }}', '{{ $notification->data['url'] ?? '#' }}', $event)"
                        class="w-full text-left flex items-center px-4 py-3 border-b hover:bg-gray-100 -mx-2">
